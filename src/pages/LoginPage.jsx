...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // The API response matches { success: true, data: { user, accessToken, ... } }
      login(data.data.user, data.data.accessToken);
      navigate("/dashboard");
    } catch (err) {
      if (err.isCanceled) return; // ignore unmount errors
  ...
  return (
    <main className="relative z-10 w-full min-h-screen flex items-center justify-center p-6 bg-background text-on-surface overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-[440px]">
        <div className="flex flex-row align-center gap-2 items-center mb-10 text-center">
